...
library(ggplot2)
# Loading data
life_expectancy <- read.csv('datasets/UNdata.csv', header=T)
...
ggplot(subdata, aes(Male, Female)) + geom_point()+  geom_abline(intercept=0, slope=1, linetype=2)  +
scale_x_continuous(limits=c(35,85)) + scale_y_continuous(limits=c(35,85))
...
ggplot(subdata, aes(x=Male, y=Female, label = Country.or.Area))+
  ...
  labs(title="Life Expectancy at Birth by Country",
       ...
       x="Males",
       y="Females")+
    geom_text(data=top_male, size=3) + geom_text(data=top_female, size=3)+ theme_bw()
